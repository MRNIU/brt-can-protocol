...
- 协议模块按职责命名；说明书版本、协议依据与冲突裁决集中在 README，不把说明书版本当作固件版本。
- 库只转换协议值与 CAN 帧，不添加外设访问、收发、任务、时序、重试、状态机、标定、物理换算、限幅或控制许可。
- 保留标准／扩展 ID、实际 RTR／FD 形态、未知状态码与完整数据位；不能把 FD 丢弃标志后送入 Classic 入口。
- 扩展 CAN ID 与 DEVICE_ID 保持显式的独立字段；厂商说明扩展地址模式的 DEVICE_ID 取 CAN ID 低八位，但未给出适用型号和固件范围。库不自动推导或校验该关系，也不推定广播语义；调用方负责地址变化后的匹配和操作结果判断。
